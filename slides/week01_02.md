...
<img
  src="./images/spatial-package-growth.png"
  alt="Growth in spatial analysis R packages over time, showing rapid expansion after 2015"
  width="800"
/>

[R for Geospatial Analysis – Spatial Ecosystem](https://geocompr.robinlovelace.net/intro.html#rs-spatial-ecosystem) (for more)
...
- How do GIS and GIScience differ from Spatial Analysis?
...
```r
# what kind of tests are these? And what are they doing?
x / 2 == 0   
x > 2
x == 7
```
...
- A vector is a 1-D ordered collection of values
- you designate a vector in R with ```c()```
...
# Much of R is built on vectors, but a lot is also built on lists

### Lists can mix and match data types

```r
mylist <- list(6, 41, "banana")
```
### Vectorized operations don't work on lists...

---

# Accessing elements from a vector or a list
...
```r
#install it
install.packages("tidyverse") # Quotes here
...
### once it's in memory, you can call a function from that package directly
...
### the syntax is a bit weird, so let's break it down
```r

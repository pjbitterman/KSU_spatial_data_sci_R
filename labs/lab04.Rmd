...
**Before writing any code, read the metadata file at `../data/wildcard_thursday/erie_raster_ex/erie_README.txt`.** This is not optional. Understanding your data source, its scaling, and its data key is essential for correct analysis.
...
Use `list.files()` with `pattern = "\\.tif$"` and `full.names = TRUE` to get the file paths for all raster images in the `images/` directory. Store the result in an object called `raster_files`. Print the result — how many files are there?
...
*Tip: using `lapply()` over your `raster_files` vector, combined with your preprocessing functions, can make this more efficient.*
...
## Task 5: Automation with functions

### 5.1 — Write `bloom_summary()`

Write a function called `bloom_summary(raster_path, aoi)` that:

1. Reads a raster from `raster_path` using `terra::rast()`
2. Applies `clean_cicyano()` and `dn_to_ci()`
3. Crops and masks to the `aoi` sf object
4. Returns a **named list** containing:
   - `date`: a string parsed from the filename (the `MMDD_MMDD` portion)
   - `mean_ci`: mean CI value within the AOI (use `terra::global()`, `na.rm = TRUE`)
   - `max_ci`: maximum CI value within the AOI
   - `bloom_cell_count`: number of non-NA cells in the masked raster (use `terra::global()` with `"notNA"`)

```{r bloom_summary_function, echo=TRUE, message=TRUE}
# your code here
```

### 5.2 — Apply across all rasters

Using `purrr::map()` or `lapply()`, apply `bloom_summary()` to **all seven rasters** in `raster_files`, using the Sandusky Bay AOI. Convert the resulting list to a data frame using `dplyr::bind_rows()`.

Print the resulting data frame. On which date was the Sandusky Bay bloom the largest (by cell count)? The most intense (by mean CI)?

```{r apply_function, echo=TRUE, message=TRUE}
# your code here
```
...
2. Cyanobacteria blooms are affected by temperature, nutrient runoff, and lake circulation. Based on the temporal patterns you observed in Task 3, does the timing of peak bloom match what you would expect ecologically? Explain your reasoning.
...
4. The `bloom_summary()` function you wrote in Task 5 is reusable across any raster and AOI. What additional arguments might you add to make it more flexible? What error handling might be useful?

---

## Bonus (+30 points)

Extend your Task 5.2 analysis: apply `bloom_summary()` across **all rasters** AND **all AOIs**, producing a comprehensive summary data frame with columns for `date`, `aoi_name`, `mean_ci`, `max_ci`, and `bloom_cell_count`.

Use this data frame to create a **faceted `ggplot2` visualization** — one panel per AOI — showing how bloom intensity (your choice of metric) varies over the season. Which AOI shows the most dramatic temporal variation?

*Tip: `purrr::map2()` or nested `lapply()` calls can help iterate over two vectors simultaneously.*

...
This lab builds on many of the discussions and exercises from class. This lab also builds on Chapters 1-3 from your textbook, as well as R for Data Science by Hadley Wickham and Garrett Grolemund (https://r4ds.hadley.nz)
...
```
Note that because we're using a spatial data frame in the `sf` package, the geometry is preserved, even though we didn't specify it. We can also omit of attributes we DON'T want (but not the geometry attribute) using a `-` flag. For example, the following code "drops" the `NAME10` attribute:
...
```{r group-ex, echo=TRUE, message=TRUE}
...
d.counties %>% 
  ...
  geom_boxplot(aes(fill = as.factor(STATEFP10)))
```
Or:
...
d.counties %>% 
  ...
  geom_histogram(aes(fill = as.factor(STATEFP10))) +
...
1.1 For each county, calculate its percent of the county's land area as the percent of the total area (land + water) for that state. land_county / (land_state + water_state).
...
`c(1, 0, -1), c(10, 1000, 100), c(.1, .001, 1e8), c("a", "b", "c")`
